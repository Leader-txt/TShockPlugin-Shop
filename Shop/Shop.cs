...
namespace Shop
{
    ...
    {
        public static bool English => Terraria.Localization.Language.ActiveCulture.LegacyId == 1;

        public const string ConfigPath = "tshock\\shop.json";
        /// <summary>
        ...
        /// </summary>
        public override string Description => English? "Make the store can be truely used." : "实现商店功能";
        ...
        /// </summary>
        public override Version Version => new Version(3, 1uage, 0, 0);
            ...
            try
            {
                ...
                foreach (var c in Change.GetChanges(player.Name))
                {
                    if (c.Account == -2)
                    {
                        ...
                        player.SendSuccessMessage(English? "Your item has been returned,please check." : "您的商品已退回,请查收");
                        c.Delete();
                    }
                    if (c.Account == -1)
                    {
        ...
        string[] EnglishHelpTextForAdmin = new string[]
        {
        ...
        { 
            try
            {
                if (args.Parameters.Count() == 0 || args.Parameters[0] == "help")
                {
                    ...
                    {
                        args.Player.SendInfoMessage(English? "/shop life,buy life" : "/shop life,血量购买");                
                    }
                ...
                switch (args.Parameters[0])
                {
                    case "change":
                        if (args.Parameters.Count() == 1 || args.Parameters[1] == "help")
                        {
                            ...
                            if (args.Player.HasPermission(Permission))
                            {
                                args.Player.SendInfoMessage(English ? "/shop change del all/sellID [nr],delete selling item,nr:don't return the item,if empty,return." : "/shop change del all/商品编号 [nr],删除商品,是否归还商品,若不归还,请填写nr,默认归还");
                            }
                            return;
                        }
                        switch (args.Parameters[1])
                        {
                            case "del":
                                if (!args.Player.HasPermission(Permission))
                                {
                                    args.Player.SendErrorMessage(English ? "You don't have the promission to use this command!" : "您无权限使用此指令!"); return;
                                }
                                        ...
                                        foreach (var c in changes)
                                        {
                                            ...
                                            foreach (var p in TShock.Players)
                                            {
                                                if (p.Name == c.Owner)
                                                {
                                                    ...
                                                    p.SendSuccessMessage(English ? "Your item has returned to your package,please check." : "您的商品已退回,请查收");
                                                    c.Delete();
                                                }
                                            }
                                        }
                            ...
                            case "cancel":
                                if (args.Parameters[2] == "all")
                                {
                                    ...
                                    args.Player.SendSuccessMessage(English ? "All of your items have been returned,please check." : "已归还全部商品,请查收");
                                    return;
                                }
                                ...
                                cha.Delete();
                                args.Player.SendSuccessMessage(English ? "Your item has been returned to your package,please check." : "已归还商品,请查收"); break;
                            case "buy":
                                int account = int.Parse(args.Parameters[2]);
                                if (account < 0)
                                {
                                    args.Player.SendErrorMessage(English ? "Sorry,the item is selling out :(,please view the other items :)" : "对不起,此商品已售出,请浏览其他商品吧~");
                                    return;
                                }
                                Change ch = Change.GetChange(account);
                                if (bank.Money < ch.Prize)
                                {
                                    args.Player.SendErrorMessage(English ? "You don't have enough money! :(" : "余额不足!");
                                    return;
                                }
                                args.Player.GiveItem(ch.NetID, ch.Stack, ch.Perfix);
                                bank.Money -= ch.Prize;
                                bank.Save();
                                args.Player.SendSuccessMessage(English ? "The item you bought has reached,please check,your rest money:" : "您的商品已到账,请查收,您当前余额为:" + bank.Money);
                                ch.Delete();
                                ch.Account = -1;
                                ch.Save();
                                foreach (var p in TShock.Players)
                            ...
                            case "add":
                                ...
                                if (item.stack == 0)
                                {
                                    args.Player.SendErrorMessage(English ? "Sorry,you can't sell empth item! :(" : "抱歉,您无法出售空的物品");
                                    return;
                                }
                    ...
                    case "life":
                        ...
                        if (args.Parameters.Count() == 1 || args.Parameters[1] == "help")
                        {
                            ...
                            if (args.Player.HasPermission(Permission))
                            {
                                args.Player.SendInfoMessage(English ? "/shop life edit life [PlayerName],edit the life,if PlayerName is empty,edit yourself." : "/shop life edit 血量 [玩家名字],修改血量,不填玩家名字则为自己");
                            }
                            return;
                        }
                    ...
                    case "clear":
                        if (!args.Player.HasPermission(Permission))
                        {
                            args.Player.SendErrorMessage(English ? "Sorry,you don't have the promission to use the command! :(" : "抱歉，您无权限使用此命令！");
                            return;
                        }
                        if (args.Parameters[1] == "bank")
                        {
                            Data.Command("delete from bank");
                            args.Player.SendSuccessMessage(English ? "Delete successfully! :)" : "删除成功!"); }
                    ...
                    case "del":
                        ...
                        {
                            args.Player.SendErrorMessage(English ? "Sorry, we can't find this item. :(" : "查无此项");
                        }
                        break;
                    case "add":
                        if (!args.Player.HasPermission(Permission))
                        ...
                        {
                            Item.GetItem(int.Parse(args.Parameters[1]));
                            args.Player.SendErrorMessage(English? "Sorry,you can't add the same item. :(" : "无法添加相同的项");
                        }
                        catch
                        {
                            ...
                            item.Save();
                            args.Player.SendSuccessMessage(English? "Adding successfully!" : "添加成功");
                        }
                        break;
                    case "list":
                        ...
                        result += English? "Your rest money is" : "您的余额为" + bank.Money;
                    ...
                    case "buy":
                        ...
                        if (num < 0)
                        {
                            args.Player.SendErrorMessage(English? "The number can't less than zero!" : "数量不能为负！");
                            return;
                        }
                        ...
                        if (ite.Prize * num < bank.Money)
                        {
                            ...
                            args.Player.SendSuccessMessage(English? "The item you bought has reached,please check.":"您购买的商品已到账，请查收");
                        }
                        else
                        {
                            args.Player.SendErrorMessage(English? "You don't have enough money!You still need" : "余额不足，还差" + (ite.Prize * num - bank.Money));                        }
                        break;
                    case "check":
                        if (args.Parameters.Count() == 1)
                            args.Player.SendSuccessMessage(English? "Your rest money is" : "您的余额为" + bank.Money);
                        else
                            args.Player.SendSuccessMessage(args.Parameters[1] + (English?"\'s rest money is":"的余额为") + Data.GetBank(args.Parameters[1]).Money);
                        break;
                    case "pay":
                        ...
                        {
                            args.Player.SendErrorMessage(English? "There isn't enough coins in your package." : "背包中钱币不足");                            return;
                        }
                        else
                        {
                            ...
                            args.Player.SendSuccessMessage(English? "The recharge has reached，please check，your rest money is" : "充值已到账，请查收，您的余额为" + bank.Money);
                        }
                        break;
                }
            }
            catch (Exception ex)
            {
                if (English)
                {
                    ...
                    TShock.Log.ConsoleError(args.Player.Name + " excuted command returned an error:");

                }
                else
                {
                    args.Player.SendErrorMessage("命令执行错误，您输入的似乎不正确");
                    TShock.Log.ConsoleError(args.Player.Name + "执行的命令报错如下");
                }
